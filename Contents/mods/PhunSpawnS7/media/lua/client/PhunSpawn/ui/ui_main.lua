...
local profileName = "PhunSpawnSelectorUI"
local UI = ISPanelJoypad:derive(profileName);

PS.ui = PS.ui or {}
PS.ui.main = PS.ui.main or {}
PS.ui.main.instances = PS.ui.main.instances or {}
PS.ui.main = ISPanelJoypad:derive(profileName);

local UI = PS.ui.main
local csystem = nil

function UI:getCityBounds(city)
...
function UI.OnOpenPanel(playerObj)
    ...
    if not instance then
        ...
        instance = UI.instances[playerIndex]
    end
...
function UI:rebuild(spawnpoints)
    ...
    local showAnyway = (isAdmin() or isDebugMode()) and self.showAll.selected[1] == true
    for k, spawn in pairs(data) do

        -- is this spawn enabled
        local enable = spawn.enabled ~= false

        -- if it isn't auto discovered, then check that the player has discovered it
        if spawn.autoDiscovered ~= true then
            enable = discoveries[k] == true
        end

        -- if enabled or in admin mode and has ticked all
        if enable or showAnyway then

            local cityName = csystem:getDefaultCityName(spawn.x, spawn.y)

            local zone = PZ:getLocation(spawn.x, spawn.y)
...
function UI:refreshCitiesCombo()
    self.city:clear()
...
function UI:setSelectedCityByKey(key)
...
function UI:setSelectedCity(cityIndex)
...
function UI:setSelectedLocation(locationIndex)
...
function UI:doTele(destinationX, destinationY, destinationZ)
            ...
            for itSq = 0, squares:size() - 1, 1 do
                ...
                for i = squareToCheck:getMovingObjects():size(), 1, -1 do
                    ...
                    if instanceof(testZed, "IsoZombie") then
                        ...
                        sendClientCommand(PS.name, PS.commands.killZombie, {
                            id = onlineID
                        })
...
function UI:exitRoom(destinationTitle, destinationCity, destinationX, destinationY, destinationZ)
    ...
    if isAdmin() then
        ...
        self.showAll.changeOptionMethod = function()
            if self.showAll.selected[1] then
                self:rebuild(PS:getAllSpawnPoints())
